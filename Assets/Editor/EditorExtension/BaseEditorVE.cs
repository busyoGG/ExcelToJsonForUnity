...
namespace EditorUIExtension
{
    public class BaseEditorVE<T> : EditorWindow where T : EditorWindow
    {
        ...
        /// 字典元素值改变监听函数
        /// </summary>
        private Dictionary<string, Action> _listValueChangeActions = new Dictionary<string, Action>();
                ...
                if (editor != null)
                {
                    ...
                    if (isList)
                    {
                        ...
                        for (int i = 0; i < (value as IList).Count; i++)
                        {
                            ...
                            Action<object> setData = res =>
                            {
                                (value as IList)[index] = res;
                                _listValueChangeActions.TryGetValue(member.Name, out var action);
                                action?.Invoke();
                            };
            ...
            Action<object> setData, bool isList = false)
        {
            ...
            switch (eType)
            {
                ...
                case EType.Class:
                    ...
                    break;
                case EType.ListValueListener:
                    if (data == null)
                    {
                        data = Delegate.CreateDelegate(typeof(Action), this, member as MethodInfo);
                    }

                    _listValueChangeActions.TryAdd(eName?.GetName(), (Action)data);
                    break;
            }
